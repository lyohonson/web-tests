...
public class LstShortener {
  ...
  private static final String baseBath = "https://lst.to/api/v1";
  ...
  public static String getShortLink(String url) {
    ...
    LOG.info("Get short link from https://lst.to/");
    ...
    LOG.info("Delete short link from https://lst.to/");
    Response deleteResponse = rest(baseBath)
        .given()
        .contentType(ContentType.JSON)
        .header("X-AUTH-TOKEN", token)
        .when()
        .delete("/link/" + shortLink.replace("https://lst.to/", ""));
